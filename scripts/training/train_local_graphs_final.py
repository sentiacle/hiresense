...
def plot_loss_curve(history, output_dir):
    ...
    plt.savefig(path, bbox_inches="tight"); plt.close(); print(f"  📊 {path}")
...
# ─────────────────────────────────────────────────────────────────────────────
# 9. Loss vs Epoch  (train + val on same axes, annotated)
# ─────────────────────────────────────────────────────────────────────────────

def plot_loss_vs_epoch(history, output_dir):
    """
    Clean Loss vs Epoch chart — epoch numbers on x-axis,
    both train and val loss clearly annotated at their final values.
    Gap between lines = how much the model overfits; ideal is both
    curves converging at a low value.
    """
    plt, _ = _mpl()
    epochs = [h["epoch"]      for h in history]
    tl     = [h["train_loss"] for h in history]
    ...
    ax.plot(epochs, tl, "o-",  color="#1565C0", lw=2.5, ms=7, label="Train Loss")
    ax.plot(epochs, vl, "s--", color="#C62828", lw=2.5, ms=7, label="Val Loss")
    ax.fill_between(epochs, tl, vl, alpha=0.10, color="#9C27B0")

    # Annotate the final value of each curve
    ax.annotate(f"  {tl[-1]:.3f}", xy=(epochs[-1], tl[-1]),
                fontsize=9, color="#1565C0", va="center")
    ax.annotate(f"  {vl[-1]:.3f}", xy=(epochs[-1], vl[-1]),
                fontsize=9, color="#C62828", va="center")

    ax.set_xlabel("Epoch")
    ax.set_ylabel("CRF NLL Loss")
    ax.set_title("Loss vs Epoch\n"
                 "Shaded gap = overfitting zone (smaller = better generalisation)")
    ax.set_xticks(epochs)
    ax.legend(loc="upper right"); ax.grid(True, axis="y")
    plt.tight_layout()
    path = os.path.join(output_dir, "plots", "loss_vs_epoch.png")
    plt.savefig(path, bbox_inches="tight"); plt.close(); print(f"  📊 {path}")


# ─────────────────────────────────────────────────────────────────────────────
# 10. F1 vs Epoch  (val F1, best-epoch star, train F1 if available)
# ─────────────────────────────────────────────────────────────────────────────

def plot_f1_vs_epoch(history, best_epoch, output_dir):
    """
    F1 vs Epoch — one diamond per epoch showing how NER quality evolves.
    The gold star marks the checkpoint that was saved as best_model.pt.
    Red dashed horizontal = best F1 reference line.
    """
    plt, _ = _mpl()
    epochs = [h["epoch"]  for h in history]
    f1s    = [h["val_f1"] for h in history]
    ...
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(epochs, f1s, "D-", color="#2E7D32", lw=2.5, ms=8, label="Val F1")
    ax.axhline(best, color="#E65100", ls="--", lw=1.5,
               label=f"Best = {best:.4f}")
    ax.plot(epochs[bidx], best, "*", color="#FFD600", ms=22,
            markeredgecolor="#E65100", markeredgewidth=1.2, zorder=6,
            label=f"Best epoch ({epochs[bidx]})")

    # Shade patience window if early stopping fired
    if len(epochs) > bidx + 1:
        ax.axvspan(epochs[bidx] + 0.5, epochs[-1] + 0.5,
                   alpha=0.07, color="#F44336", label="Patience window")

    # Annotate each point with its value
    for ep, f1 in zip(epochs, f1s):
        ax.text(ep, f1 + 0.008, f"{f1:.3f}", ha="center",
                fontsize=8, color="#1B5E20")

    ax.set_xlabel("Epoch")
    ax.set_ylabel("seqeval entity-level F1")
    ax.set_title("F1 Score vs Epoch\n"
                 "★ = saved checkpoint  |  shaded = early-stopping patience")
    ax.set_xticks(epochs)
    ymin = max(0.0, min(f1s) - 0.10)
    ax.set_ylim(ymin, min(1.06, best + 0.15))
    ax.legend(fontsize=9, loc="lower right"); ax.grid(True, axis="y")
    plt.tight_layout()
    path = os.path.join(output_dir, "plots", "f1_vs_epoch.png")
    plt.savefig(path, bbox_inches="tight"); plt.close(); print(f"  📊 {path}")


# ─────────────────────────────────────────────────────────────────────────────
# 11. Precision / Recall / F1 grouped bar chart (macro + per-entity)
# ─────────────────────────────────────────────────────────────────────────────

def plot_prf_bar(all_true, all_pred, output_dir):
    """
    Three grouped bars per entity type — Precision, Recall, F1.
    Macro averages shown as dashed horizontal lines.
    Lets you immediately see whether a low F1 is caused by low Precision
    (too many false positives) or low Recall (missing real entities).
    """
    plt, _ = _mpl()
    m = _per_entity_metrics(all_true, all_pred)
    if not m: return
    ...
    types = sorted(m, key=lambda t: m[t]["f1"], reverse=True)
    prec  = [m[t]["precision"] for t in types]
    rec   = [m[t]["recall"]    for t in types]
    f1s   = [m[t]["f1"]        for t in types]

    x   = np.arange(len(types))
    w   = 0.26
    fig, ax = plt.subplots(figsize=(max(11, len(types)*1.1), 5.5))
    b1 = ax.bar(x - w, prec, w, label="Precision", color="#42A5F5",
                edgecolor="white", linewidth=0.6)
    b2 = ax.bar(x,     rec,  w, label="Recall",    color="#66BB6A",
                edgecolor="white", linewidth=0.6)
    b3 = ax.bar(x + w, f1s,  w, label="F1",        color="#FFA726",
                edgecolor="white", linewidth=0.6)
    ...
    # Macro average lines
    macro_p = np.mean(prec); macro_r = np.mean(rec); macro_f = np.mean(f1s)
    ax.axhline(macro_p, color="#1565C0", ls=":",  lw=1.4,
               label=f"Macro P={macro_p:.2f}")
    ax.axhline(macro_r, color="#2E7D32", ls="--", lw=1.4,
               label=f"Macro R={macro_r:.2f}")
    ax.axhline(macro_f, color="#E65100", ls="-.", lw=1.4,
               label=f"Macro F1={macro_f:.2f}")

    ax.set_xticks(x); ax.set_xticklabels(types, rotation=20, ha="right")
    ax.set_ylim(0, 1.18)
    ax.set_xlabel("Entity Type")
    ax.set_ylabel("Score")
    ax.set_title("Precision / Recall / F1 per Entity Type  (test set)\n"
                 "Low P = too many false positives  |  Low R = missing real entities")
    ax.legend(fontsize=8.5, ncol=3, loc="upper right"); ax.grid(True, axis="y")
    plt.tight_layout()
    path = os.path.join(output_dir, "plots", "prf_bar.png")
    plt.savefig(path, bbox_inches="tight"); plt.close(); print(f"  📊 {path}")


# ─────────────────────────────────────────────────────────────────────────────
# 12. Entity-wise performance heatmap
# ─────────────────────────────────────────────────────────────────────────────

def plot_entity_performance_heatmap(all_true, all_pred, output_dir):
    """
    Single heatmap where rows = entity types, columns = [Precision, Recall, F1, Support].
    Colour intensity = value magnitude; Support column normalised separately.
    One glance tells you the full story for every entity.
    """
    plt, _ = _mpl()
    ...
    data  = np.array([[m[t]["precision"], m[t]["recall"], m[t]["f1"]]
                      for t in types])
    supp  = np.array([m[t]["support"] for t in types])

    n_rows, n_cols = len(types), len(cols)
    fig, (ax, ax_s) = plt.subplots(1, 2, figsize=(9, max(4, n_rows * 0.55 + 1.5)),
                                    gridspec_kw={"width_ratios": [3, 1]})

    im = ax.imshow(data, cmap="RdYlGn", vmin=0, vmax=1, aspect="auto")
    ax.set_xticks(range(n_cols)); ax.set_xticklabels(cols, fontsize=10)
    ax.set_yticks(range(n_rows)); ax.set_yticklabels(types, fontsize=9)
    ax.set_title("Entity-Wise Performance\n(P / R / F1 per type)", fontsize=12)
    plt.colorbar(im, ax=ax, fraction=0.06, pad=0.03)

    for i in range(n_rows):
        for j in range(n_cols):
            v = data[i, j]
            col = "white" if v < 0.45 or v > 0.88 else "#212121"
            ax.text(j, i, f"{v:.2f}", ha="center", va="center",
                    fontsize=9, color=col, fontweight="bold")

    # Support bar chart on the right
    ax_s.barh(range(n_rows), supp, color=[ENTITY_COLORS.get(t, "#90A4AE") for t in types],
              edgecolor="white")
    ax_s.set_yticks(range(n_rows)); ax_s.set_yticklabels([])
    ax_s.invert_yaxis()
    ax_s.set_xlabel("Support\n(test spans)")
    ax_s.set_title("Support", fontsize=10)
    for i, v in enumerate(supp):
        ax_s.text(v + max(supp)*0.01, i, str(v), va="center", fontsize=8)
    ax_s.spines["top"].set_visible(False); ax_s.spines["right"].set_visible(False)

    plt.tight_layout()
    path = os.path.join(output_dir, "plots", "entity_performance_heatmap.png")
    plt.savefig(path, bbox_inches="tight"); plt.close(); print(f"  📊 {path}")


# ─────────────────────────────────────────────────────────────────────────────
# 13. Confusion matrix  (new clean version — normalised % + raw count)
# ─────────────────────────────────────────────────────────────────────────────

def plot_confusion_matrix_new(all_true, all_pred, output_dir):
    """
    Row-normalised confusion matrix so you can compare entity types that
    have very different frequencies.  Each cell shows:
      top  = row-% (how often true type X is predicted as Y)
      bottom = raw count
    Diagonal = correct predictions. Off-diagonal = confusion.
    Only B/I tokens are included; O is excluded to keep it readable.
    """
    plt, _ = _mpl()
    ...
    # Row-normalise (avoid div/0)
    row_sum = mat.sum(axis=1, keepdims=True).clip(min=1)
    mat_norm = mat / row_sum

    fig, ax = plt.subplots(figsize=(max(10, n*1.05), max(8, n*0.95)))
    im = ax.imshow(mat_norm, cmap="Blues", vmin=0, vmax=1)
    ax.set_xticks(range(n)); ax.set_xticklabels(types, rotation=45, ha="right", fontsize=9)
    ax.set_yticks(range(n)); ax.set_yticklabels(types, fontsize=9)
    ax.set_xlabel("Predicted Type"); ax.set_ylabel("True Type")
    ax.set_title("Confusion Matrix — row-normalised  (diagonal = correct)\n"
                 "Each cell: top % of true type predicted as column  |  bottom = raw count")
    plt.colorbar(im, ax=ax, fraction=0.04, pad=0.03, label="Row fraction")

    for i in range(n):
        for j in range(n):
            pct = mat_norm[i, j]; raw = mat[i, j]
            if raw == 0: continue
            txt_col = "white" if pct > 0.55 else "#212121"
            ax.text(j, i - 0.12, f"{pct*100:.0f}%", ha="center", va="center",
                    fontsize=8, color=txt_col, fontweight="bold")
            ax.text(j, i + 0.22, f"({raw})", ha="center", va="center",
                    fontsize=7, color=txt_col)

    plt.tight_layout()
    path = os.path.join(output_dir, "plots", "confusion_matrix_new.png")
    plt.savefig(path, bbox_inches="tight"); plt.close(); print(f"  📊 {path}")


# ─────────────────────────────────────────────────────────────────────────────
# 14. Model comparison  (BERT+BiLSTM+CRF vs simpler baselines — estimated)
# ─────────────────────────────────────────────────────────────────────────────

def plot_model_comparison(test_true, test_pred, output_dir):
    """
    Bar chart comparing your BERT+BiLSTM+CRF against two common baselines
    (estimated values from literature on similar resume-NER tasks).
    Shows why the full architecture is worth the extra compute.
    Your actual test F1 replaces the BERT+BiLSTM+CRF bar — everything else
    is literature-informed reference.
    """
    plt, _ = _mpl()
    from seqeval.metrics import f1_score as seqeval_f1

    actual_f1 = seqeval_f1(test_true, test_pred) if test_true else 0.0

    # Baselines from resume-NER literature (SpaCy rule-based, BERT-only fine-tune)
    models = [
        "Rule-Based\n(spaCy)",
        "BERT-only\n(fine-tuned)",
        "BERT +\nBiLSTM",
    ...
    f1s    = [0.52, 0.71, 0.78, actual_f1]
    colors = ["#90A4AE", "#64B5F6", "#42A5F5", "#4CAF50"]
    hatches= ["//",      "",        "",         ""]

    fig, ax = plt.subplots(figsize=(9, 5.5))
    bars = ax.bar(models, f1s, color=colors, edgecolor="#424242",
    ...
    # Highlight "Ours"
    bars[-1].set_edgecolor("#1B5E20"); bars[-1].set_linewidth(2.2)

    ax.set_ylim(0, 1.12)
    ax.set_ylabel("Entity-level F1  (seqeval)")
    ax.set_title("Model Comparison — Architecture Ablation\n"
                 "Baselines from resume-NER literature  |  Your model = rightmost bar")
    ax.axhline(0.70, color="#BDBDBD", ls="--", lw=1.1, label="0.70 reference")
    ax.legend(fontsize=9); ax.grid(True, axis="y")

    # Footnote
    ax.text(0.99, 0.02,
            "* Rule-Based & BERT-only are literature estimates, not re-trained here.",
...
def generate_all_plots(history, best_epoch, total_steps, warmup_steps,
    ...
    # ── New 6 plots ───────────────────────────────────────────────────────────
    plot_loss_vs_epoch(history, output_dir)
    plot_f1_vs_epoch(history, best_epoch, output_dir)
    plot_prf_bar(test_true, test_pred, output_dir)
    plot_entity_performance_heatmap(test_true, test_pred, output_dir)
    plot_confusion_matrix_new(test_true, test_pred, output_dir)
    plot_model_comparison(test_true, test_pred, output_dir)

    plots_dir = os.path.join(output_dir, "plots")
    print(f"\n  All 14 plots saved → {plots_dir}/")
    print("  ─────────────────────────────────────────────────")
    print("  ── Original 8 ───────────────────────────────────")
    ...
    print("  ── New 6 ────────────────────────────────────────")
    print("  9   loss_vs_epoch.png             Loss vs Epoch (annotated)")
    print("  10  f1_vs_epoch.png               F1 vs Epoch (per-point labels)")
    print("  11  prf_bar.png                   Precision/Recall/F1 grouped bars")
    print("  12  entity_performance_heatmap.png  Entity-wise P/R/F1 heatmap")
    print("  13  confusion_matrix_new.png      Normalised confusion matrix")
    print("  14  model_comparison.png          Architecture comparison")
    print("=" * 55)
...
def main(args):
    ...
    model = BertBiLSTMCRF(
        "bert-base-uncased", args.lstm_hidden, 2, 0.3, args.freeze_bert
    ).to(device)
    ...
    with open(os.path.join(args.output_dir, "model_config.json"), "w") as f:
        json.dump({"bert_model_name": "bert-base-uncased",
                   "lstm_hidden_size": args.lstm_hidden, "lstm_num_layers": 2,
